const transactionTable = {
    populateTransactionsTable: (transactions, paginationData = null) => {
        ...
        if (transactions && transactions.length > 0) {
            transactions.forEach(transaction => {
                ...
                row.innerHTML = `
                    ...
                    <td>${formatters.formatCurrency(transaction.amount)}</td>
                    <td>${transaction.category || 'N/A'}</td>
                    <td>${formatters.formatTransactionType(transaction.transaction_type) || 'N/A'}</td>
                    <td>${formatters.formatBoolean(transaction.essential_expense)}</td>
                ...
                if (deleteButton) {
                    deleteButton.addEventListener('click', () => {
                        transactionHandlers.handleDeleteTransaction(transaction.id);
                    });
                }

                if (editButton) {
                    editButton.addEventListener('click', () => {
                        transactionHandlers.handleEditTransaction(transaction.id);
                    });
                }
            });
        } else {
        ...
        if (paginationData) {
            ...
            paginationControls.innerHTML = `
                <ul class="pagination mb-0">
                    ...
                    ${transactionTable.generatePageNumbers(currentPage, totalPages)}
